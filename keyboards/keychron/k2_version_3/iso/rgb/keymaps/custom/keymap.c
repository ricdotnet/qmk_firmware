...
// parenthesis in shift keys

void lsh_paren_finished(tap_dance_state_t *state, void *user_data) {
    if (state->pressed) {
        // Held → act as Left Shift
        register_code(KC_LSFT);
    } else {
        // Tap → send '('  (Shift + 9 on UK)
        tap_code16(S(KC_9));
    }
}

void lsh_paren_reset(tap_dance_state_t *state, void *user_data) {
    unregister_code(KC_LSFT);
}

void rsh_paren_finished(tap_dance_state_t *state, void *user_data) {
    if (state->pressed) {
        // Held → act as Right Shift
        register_code(KC_RSFT);
    } else {
        // Tap → send ')' (Shift + 0 on UK)
        tap_code16(S(KC_0));
    }
}

void rsh_paren_reset(tap_dance_state_t *state, void *user_data) {
    unregister_code(KC_RSFT);
}

// backspace in caps key

void caps_bspc_finished(tap_dance_state_t *state, void *user_data) {
...
void caps_bspc_reset(tap_dance_state_t *state, void *user_data) {
    // nothing needed
}

tap_dance_action_t tap_dance_actions[] = {
    [TD_LSH_PAREN] = ACTION_TAP_DANCE_FN_ADVANCED(
        NULL,
        lsh_paren_finished,
        lsh_paren_reset
    ),
    [TD_RSH_PAREN] = ACTION_TAP_DANCE_FN_ADVANCED(
        NULL,
        rsh_paren_finished,
        rsh_paren_reset
    ),
    [TD_CAPS_BSPC] = ACTION_TAP_DANCE_FN_ADVANCED(
        NULL,
        caps_bspc_finished,
        caps_bspc_reset
    ),
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_iso_85(
        ...
        TD(TD_CAPS_BSPC),  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,     KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,   KC_NUHS,  KC_ENT,             KC_HOME,
        TD(TD_LSH_PAREN),  KC_NUBS,  KC_Z,     KC_X,     KC_C,     KC_V,     KC_B,     KC_N,     KC_M,     KC_COMM,  KC_DOT,   KC_SLSH,             TD(TD_RSH_PAREN),  KC_UP,    KC_END,
